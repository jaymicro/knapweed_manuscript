...
df_biomass <- read.csv("rf_biomass.csv" ) %>% 
  janitor::clean_names() %>% 
  drop_na()
...
df_biomass %>% group_by(plant_community) %>% summarise(median(biomass))
...
df_nut %>% group_by(plant_community) %>% summarise(median(c_result))
# Plot carbon data --------------------------------------------------------
...
df_nut %>% group_by(plant_community) %>% summarise(median(n_result))
...
```{r, fig.height= 4, fig.width=8}
rf_biom <- read.csv("rf_biomass.csv") %>% 
  janitor::clean_names() %>% 
...
cn_df <- read.csv("soil_cn.csv") %>% 
  janitor::clean_names() %>% 
  arrange(id) %>% 
  mutate(id = str_trim(id))

ele_df <- read.csv( "elements.csv" ) %>% 
  janitor::clean_names() %>% 
  arrange(code) 
